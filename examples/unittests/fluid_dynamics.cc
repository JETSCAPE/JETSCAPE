...
void test_not_in_range(EvolutionHistory hist, real tau, real x, real y,
                       real eta) {
  try {
...
TEST(EvolutionHistoryTest, TEST_WRITE) {
  ...
  ASSERT_NEAR(hist.get(0.8, 0.0, 0.0, 0.0).energy_density,
              static_cast<real>(const_ed), 1.0E-6);
}
