...
Qn_diff_pT_cuts=[0.0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1.,1.05,1.1,1.15,1.2,1.25,1.3,1.35,1.4,1.45,1.5,1.55,1.6,1.65,1.7,1.75,1.8,1.85,1.9,1.95,2.,2.1,2.2,2.3,2.4,2.5,2.6,2.7,2.8,2.9,3.,3.2,3.4,3.6,3.8,4.,10.0]
...
def calc_Qvector(pT, phi, eta, pTbins, etabins, Nharmonic):
        ...
        # calculate Qn
        results = {'N': np.zeros(shape, dtype=int_t), 'Qn': np.zeros([*shape, Nharmonic], dtype=complex_t)}
...
def run_events(args, results_file):
        ...
        def run_single_event(event_number):
                ...
                for idf in [0, 1, 2, 3]: 
                        ...
                        #THESE DATA ARE AVAILABLE FOR STAR
...
def main():
        ...
        with open(args.results, filemode + 'b') as results_file, tempfile.TemporaryDirectory( prefix='jetscape-', dir=args.tmpdir) as workdir:
                ...
                os.system( 'cp {:s}/jetscape_init.xml jetscape_init.xml'.format(prefix)  )
                os.system( 'cp {:s}/freestream_input freestream_input'.format(prefix)  )
                os.system( 'cp {:s}/music_input music_input'.format(prefix) )
                os.system( 'cp {:s}/iS3D_parameters.dat iS3D_parameters.dat'.format(prefix)  )
