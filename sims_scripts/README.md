...
# Submit job to stampede2
   ...
   The workflow for event simulation has been written into `sims-scripts/submit/run-events`. The job is submitted to the cluster by the slurm submit file `submit_simple` and `submit_launcher` under `sims-scripts/submit`. 
   The first one uses srun so that each cpu on each node is assigned a job to do, and it finishes when the last cpu finished its task.
   The second one uses the `launcher` application available on `stampede2`. 
   The `launcher` will submit a list of jobs generated by `submit_launcher` to the free cpu in your requested nodes, so it is a very efficient way to use up our allocations.
