...
void CausalLiquefier::smearing_kernel(
  ...
  if (tau - 0.5 * dtau <= tau_drop + tau_delay &&
    ...
    double delta_r2 = (x - x_drop[1]) * (x - x_drop[1]) +
...
double CausalLiquefier::causal_diffusion_kernel(double t, double r) const {
...
double CausalLiquefier::causal_diffusion_smooth(double t, double r) const {
  ...
  if (r < c_diff * t) {
    ...
    return (exp(-gamma_relax * t) / (20. * M_PI)) *
           (2. * gamma_relax * gamma_relax / c_diff) *
           (i1 / (c_diff * u) + 4. * t * i2 / u / u);
  } else {
    return 0.0;
  }
}
...
double CausalLiquefier::causal_diffusion_delta(double t, double r) const {
  ...
  if (r >= c_diff * t - r_w && r < c_diff * t) {
    ...
    return (exp(-gamma_relax * t) / (20. * M_PI)) *
           (8. - 3. * exp(-gamma_relax * t) + 2. * gamma_relax * t +
            4. * gamma_relax * gamma_relax * t * t) /
           r / r / r_w;
