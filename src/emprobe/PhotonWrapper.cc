...
RegisterJetScapeModule<PhotonWrapper> PhotonWrapper::reg("Photon");
...
void PhotonWrapper::InitTask() {

   JSINFO << "Initialize Photon module";
   bulk_info_array.clear();
   std::string input_file =
       GetXMLElementText({"EMProbe", "Photon", "Photon_input_file"});
   std::string turn_on_muB =
       GetXMLElementText({"EMProbe", "Photon", "turn_on_muB"});

   double grid_xmax =
       GetXMLElementDouble({"IS",  "grid_max_x"});
   double grid_ymax =
       GetXMLElementDouble({"IS",  "grid_max_y"});
   double grid_dx =
       GetXMLElementDouble({"IS",  "grid_step_x"});
   double grid_dy =
       GetXMLElementDouble({"IS",  "grid_step_y"});

  Photon_ptr_ = std::unique_ptr<Photon::JS_photon>(
           new Photon::JS_photon(input_file));
  Photon_ptr_->paraRdr->setVal("Xmin",-grid_xmax+0.5*grid_dx);
  Photon_ptr_->paraRdr->setVal("Ymin",-grid_ymax+0.5*grid_dy);
}

void PhotonWrapper::Exec() {
   JSINFO << "running Photon ...";
   bulk_info_array.resize(0);
   getBulkInforfromJetScape();
   photon_spec = std::make_shared<std::vector<float>>(Photon_ptr_->run(bulk_info_array));

   JSINFO << "Photon calculation finished.";
}
...
void PhotonWrapper::WriteTask(weak_ptr<JetScapeWriter> w) {
...
void PhotonWrapper::getBulkInforfromJetScape() {
