    ...
    if (pIn[i].t() > Q0 * Q0 + rounding_error ||
          ...
          time)  // it is time to split and calculate the effect of scattering
      {
        ...
        for (double el_time = initR0; el_time < time + rounding_error;
          ...
          if (ModificationFactor > 0.0){
            ModificationCorr = exp(-pow(ModificationFactor / tempLoc, ModificationPower));
            prob_el = el_CR * tempLoc * (polylog_series(3, ModificationCorr, 1.0e-4) - polylog_series(3, -ModificationCorr, 1.0e-4)) / (polylog_series(2, ModificationCorr, 1.0e-4) - polylog_series(2, -ModificationCorr, 1.0e-4));
...
double Matter::GeneralQhatFunction(int QhatParametrization, double Temperature,
  ...
  switch (QhatParametrization) {
   ...
   { 
    ModificationCorr = exp(-pow(ModificationFactor / Temperature, ModificationPower));
    qhat = qhat * 24.0 * (polylog_series(3, ModificationCorr, 1.0e-4) - polylog_series(3, -ModificationCorr, 1.0e-4)) / 50.4864;
   }
  return qhat;
}
...
void Matter::flavor(int &CT, int &KATT0, int &KATT2, int &KATT3,
  ...
  if (KATT00 == 21) {  //.....for gluon
    ...
    if (ModificationFactor > 0.0) {
      ModificationCorr = exp(-pow(ModificationFactor / tempLoc, ModificationPower));
      R1 = R1 * polylog_series(3, ModificationCorr, 1.0e-4);
      R3 = R3 * -polylog_series(3, -ModificationCorr, 1.0e-4) * 4.0 / 3.0;
    }
    double R0 = R1 + R3;
    ...
    if (ModificationFactor > 0.0) {
      ModificationCorr = exp(-pow(ModificationFactor / tempLoc, ModificationPower));
      R3 = R3 * polylog_series(3, ModificationCorr, 1.0e-4);
      R4 = R4 * -polylog_series(3, -ModificationCorr, 1.0e-4) * 4.0 / 3.0;
      R5 = R5 * -polylog_series(3, -ModificationCorr, 1.0e-4) * 4.0 / 3.0;
      R7 = R7 * -polylog_series(3, -ModificationCorr, 1.0e-4) * 4.0 / 3.0;
      R00 = R3 + R4 + R5 + R7;
    }
