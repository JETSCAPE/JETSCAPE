  ...
  for (int i = 1; i <= np; i++) {
    ...
    if (CAT[i] != 1) {
      ...
      if (free == 0) {
        ...
        if (ModificationFactor > 0.0){
          ModificationCorr = exp(-pow(ModificationFactor / T, ModificationPower));
          qhatTP = qhatTP * 24.0 * (polylog_series(3, ModificationCorr, 1.0e-2) - polylog_series(3, -ModificationCorr, 1.0e-2)) / 50.4864;
        }
...
void LBT::flavor(int &CT, int &KATT0, int &KATT2, int &KATT3, double RTE,
  ...
  if (KATT00 == 21) {  //.....for gluon
    ...
    if (ModificationFactor > 0.0){
       ...
       R1 = R1 * polylog_series(3, ModificationCorr, 1.0e-2) * debyemassq_ratio;
       R2 = R2 * polylog_series(2, ModificationCorr, 1.0e-2) / polylog_series(2, 1.0, 1.0e-2);
       R3 = R3 * -polylog_series(3, -ModificationCorr, 1.0e-2) * 4.0/3.0 * debyemassq_ratio;
    }
    ...
    if (ModificationFactor > 0.0){
       ModificationCorr = exp(-pow(ModificationFactor / T, ModificationPower));
       double debyemassq_ratio = 4.0 * (polylog_series(2, ModificationCorr) - polylog_series(2, -ModificationCorr))/ pow(pi, 2.0);
       R3 = R3 * polylog_series(3, ModificationCorr, 1.0e-2) * debyemassq_ratio;
       R4 = R4 * -polylog_series(3, -ModificationCorr, 1.0e-2) * 4.0/3.0 * debyemassq_ratio;
       R5 = R5 * -polylog_series(3, -ModificationCorr, 1.0e-2) * 4.0/3.0 * debyemassq_ratio;
       R6 = R6 * polylog_series(2, -ModificationCorr, 1.0e-2) / polylog_series(2, -1.0, 1.0e-2);
       R7 = R7 * -polylog_series(3, -ModificationCorr, 1.0e-2) * 4.0/3.0 * debyemassq_ratio;
       R8 = R8 * polylog_series(2, -ModificationCorr, 1.0e-2) / polylog_series(2, -1.0, 1.0e-2);
    }
