...
void Martini::Init() {
  ...
  if ( !martini->FirstChildElement( "alphas" ) )     throw std::runtime_error("Martini not properly initialized in XML file ...");
  ...
  if ( !martini->FirstChildElement( "recoil_on" ) )     throw std::runtime_error("Martini not properly initialized in XML file ...");
  ...
  alpha_s = 0.3;
  martini->FirstChildElement("alphas")->QueryDoubleText(&alpha_s);
  ...
  recoil_on = false;
  martini->FirstChildElement("recoil_on")->QueryIntText(&recoil_on);
