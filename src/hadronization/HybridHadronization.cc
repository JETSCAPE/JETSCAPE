...
//hardcoding turning decays off, just in case
pythia.readString("HadronLevel:Decay = off");
...
//				if(WigM[1]*recofactor2 >= 1){
				double mult = (considering[1].is_thermal()) ? th_recofactor : sh_recofactor;
				if(WigM[1]*recofactor2*mult >= rndmeson){
...
*/
					...
					//now setting if baryon is in excited state
					if(WigM[0]*recofactor2*mult < rndmeson){formedhadron.is_excited(true);}
...
void HybridHadronization::gluon_decay(HHparton& glu, parton_collection& qrks){
	...
	if(sum2 > 0.){Betag.Set(Betag.x(),Betag.y(),Betag.z(),1./sqrt(1.-sum2));}
...
bool HybridHadronization::invoke_py(){
	...
	while(need_hadronization){
		...
		for(int i=1; i<event.size(); ++i){if(event[i].isFinal() /*&& (std::abs(event[i].y()) <= 1.)*/){
			...
			bool col_known = false; bool acol_known = false;
			...
			if(!info_found){JSWARN << "  Could not find spacetime information for hadron in event!" << had_col << ":" << had_acol << " , " << col_known << ":" << acol_known; bad_evt = true;//!
		...
		if(bad_evt){
			/*std::cout << "\n\nRemn:\n";
			for(int ict=0; ict<HH_pyremn.num(); ++ict){std::cout << HH_pyremn[ict].id() << ", " << HH_pyremn[ict].PY_stat() << ", " << HH_pyremn[ict].col() << ", " << HH_pyremn[ict].acol()
				<< ", " << HH_pyremn[ict].px() << ", " << HH_pyremn[ict].py() << ", " << HH_pyremn[ict].pz() << ", " << HH_pyremn[ict].e()
				<< ", " << HH_pyremn[ict].PY_tag1() << ", " << HH_pyremn[ict].PY_tag2() << ", " << HH_pyremn[ict].PY_tag3() << "\n";}
			std::cout << "\n\nHad:\n";
			for(int ict=0; ict<HH_hadrons.num(); ++ict){std::cout << HH_hadrons[ict].id()
				<< ", " << HH_hadrons[ict].px() << ", " << HH_hadrons[ict].py() << ", " << HH_hadrons[ict].pz() << ", " << HH_hadrons[ict].e()
				<< ", " << HH_hadrons[ict].x() << ", " << HH_hadrons[ict].y() << ", " << HH_hadrons[ict].z() << ", " << HH_hadrons[ict].x_t() << "\n";}
			std::cout << "\n\n";
			event.list();*/
		}
