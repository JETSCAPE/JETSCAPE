...
void HybridHadronization::Init(){
  ...
  if (hadronization) {
	...
	if ( RandomXmlDescription ){
		...
		if( xmle ){xmle->QueryUnsignedText(&xml_uintin);}
		else{JSWARN << "No <seed> element found for <Random> element in xml, seeding to 0";}
		if(xml_uintin < std::numeric_limits<unsigned int>::max()){rand_seed = xml_uintin;} xml_intin = std::numeric_limits<unsigned int>::max();
	}
	else{
	...
	if ( XmlPythiaGun ){
		...
		if( xmle ){xmle->QueryIntText(&xml_intin);}
...
void HybridHadronization::stringprep(parton_collection& SP_remnants, parton_collection& SP_prepremn, bool cutstr){
	...
	for(int par_i=0; par_i<=SP_remnants.num(); ++par_i){
		...
		if((netqrk+netdiqrk)%3 != 0){
			...
			double fake_pT =  (p_fake >= 1.) ? 0.282842712474619 : 0.; double fake_phi = 2*pi*ran(); //sqrt(2*0.2*0.2) = sqrt(0.08) = 0.28284271247461900976033774484194
			if(HH_thermal.num() > 0){
				...
				if(qrk_close > -1){
					...
					fakeparton.id(HH_thermal[qrk_close].id()); fakeparton.orig(1); fakeparton.is_remnant(true); fakeparton.is_strendpt(true); fakeparton.string_id(prev_str); fakeparton.pos_str(-1);
					...
					fakeparton.mass(xmq); fakeparton.e(std::sqrt(fakeparton.px()*fakeparton.px()+fakeparton.py()*fakeparton.py()+fakeparton.pz()*fakeparton.pz()+fakeparton.mass()*fakeparton.mass())); //fakeparton.e(xmq);
					//will reposition fake parton within the string in the next repair section...
					++str_len; ++numqrk; netqrk += (2*std::signbit(-fakeparton.id())-1);
				}
			}
