...
void ThermalPartonSampler::sample_brick() {
  ...
  double TCachedLight =
      getClosestCachedTemp(CacheFermiDiracIntegralLight, hydroTc);
  double TCachedStrange =
      getClosestCachedTemp(CacheFermiDiracIntegralStrange, hydroTc);
...
#pragma omp parallel for reduction(+ : num_ud, num_s)
  for (int iS = 0; iS < surface.size(); ++iS) {
    ...
    double TCacheLight =
        getClosestCachedTemp(CacheFermiDiracIntegralLight, TRead);
    double TCacheStrange =
        getClosestCachedTemp(CacheFermiDiracIntegralStrange, TRead);
