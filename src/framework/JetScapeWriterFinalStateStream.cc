...
void JetScapeWriterFinalStateStream<T>::WriteEvent() {
  ...
  // We cannot write directly to the file since we don't know how many particles
  // we have in the filtered case. To resolve this, we'll write to an
  // intermediate stringstream, and then write that to file.
  std::stringstream ss;
  ...
  for (const auto& p : particles) {
    ...
    ss << ipart << " " << particle->pid() << " " << particle->pstat() << " "
  ...
  // of (filtered) particles by the end of the loop, so we can just take advnage
  // of it.
  output_file << "#"
              << "\t"
              << "Event\t"
              ...
              << (GetHeader().GetEventPlaneAngle() > -999
                      ...
                      : 0)
              << "\t"
              << "N_" << GetName() << "\t" << ipart << centrality_text
              << pt_hat_text << "\n";
...
void JetScapeWriterFinalStateStream<T>::Init() {
  ...
  if (GetActive()) {
    ...
    // Print the status codes that will be skipped for logging purposes to
    // ensure that it's clear that the values are propagated correctly.
    if (particleStatusToSkip.size() > 0) {
      std::stringstream ss;
